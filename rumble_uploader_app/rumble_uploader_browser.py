...
def upload_to_rumble(rumble_video_script_serialized_data):
    ...
    def find_file(rumble_video_file, file_path):
        try:
            ...
            # Check if the script has permissions to access the file
    ...
    try:
        ...
        file_input = WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.XPATH, "//input[@type='file']")))
        # Send the file path directly to the input element
        file_input.send_keys(rumble_video_file_upload)

        # file_input = WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.XPATH, "//input[@type='file']"))).click()
        # file_input.send_keys(rumble_video_file_upload)
